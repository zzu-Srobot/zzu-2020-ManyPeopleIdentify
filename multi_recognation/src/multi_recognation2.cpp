...
string Things[]={"herbal tea","lemon tea","water","cola","red bull","bath cream","milk tea","toothpaste","biscuits","chips","sprite","beer","napkins"};
...
const float depthThreshold = 2.7; //设置检测到的最远距离
...
int find_name(string quest_str)
{
	...
	else if(ques_find_or(quest_str, {"te","kurt","colt","cook","kat"})) return 7;// "Kurt";
...
{
	if(ques_find_or(quest_str, {"herbal","her","ball","herbal tea"}))return 0;// Herbal tea;
	else if(ques_find_or(quest_str, {"lemon","le","mo","lemon tea"})) return 1;// Lemon tea;
	else if(ques_find_or(quest_str, {"water"})) return 2;// Water;
	else if(ques_find_or(quest_str, {"cola","cold","col"})) return 3;// Cola;
	else if(ques_find_or(quest_str, {"red","red bull"})) return 4;// Red bull;
	else if(ques_find_or(quest_str, {"bath cream","bath","cream"})) return 5;// bath cream;
	else if(ques_find_or(quest_str, {"milk tea","milk"})) return 6;// milk tea;
	else if(ques_find_or(quest_str, {"toothpaste","tooth","paste","too","taste"})) return 7;// water;
	else if(ques_find_or(quest_str, {"biscuit","bis","base","cuit","cute"})) return 8;// water;
	else if(ques_find_or(quest_str, {"chips","ch","ps","trip"})) return 9;// water;
	else if(ques_find_or(quest_str, {"sprite","ride","prize","rite","right","bride","bread","cry","spread","break","fight","buy"})) return 10;// water;
	else if(ques_find_or(quest_str, {"beer"})) return 11;// water;
	else if(ques_find_or(quest_str, {"napkins","nap","kins","gins"})) return 12;// water;
...
//********************************************************************************	  
		  ...
		  int j=0;//对应minposition的position下标
		  int i=0;
		  float minposition = -1;//哨兵
...
//---------------------------------------------------------------					
					if(mindepth>0.8&&mindepth<depthThreshold+0.01)	
					{
						vel.linear.x= 0.25;
						vel.linear.y = 0;
						vel.linear.z = 0;
					}
					else if(mindepth>0&&mindepth<0.5)	
					{
						vel.linear.x= -0.2;
						vel.linear.y = 0;
						vel.linear.z = 0;
...
//---------------------------------------------------------------------					
					...
					v_pub.publish(vel);
					if(fabs(vel.linear.x) <0.01&&fabs(vel.linear.y) < 0.01 && fabs(vel.linear.z) < 0.01&&fabs(vel.angular.x)< 0.01&&fabs(vel.angular.y) < 0.01&&fabs(vel.angular.z)< 0.01&&mindepth>=0.5&&mindepth<=0.81)//找到人
							{
								for(int i=0; i<3; ++i)
									v_pub.publish(vel);
...
void introduce_robot(){
	...
	cout<<"Hello, I'm Emily. I am from the service robot laboratory of Zhengzhou University."<<endl;
}
...
//进门****************************************************************************
	...
	goal.target_pose.header.frame_id = "map";       //进场到指定位置自我介绍
	goal.target_pose.header.stamp = ros::Time::now();
	goal.target_pose.pose.position.x = 4.307;
	goal.target_pose.pose.position.y = 1.589;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(-0.2);
	...
	goal.target_pose.header.stamp = ros::Time::now();
	goal.target_pose.pose.position.x = 6.954;
	goal.target_pose.pose.position.y = 2.779;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(0.804);
	ROS_INFO("Sending goal");
	ac.sendGoal(goal);
	ac.waitForResult();
	if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) ROS_INFO("succeeded");
	...
	goal.target_pose.header.frame_id = "map";       
	goal.target_pose.header.stamp = ros::Time::now();
	goal.target_pose.pose.position.x = 7.610;
	goal.target_pose.pose.position.y = 5.566;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(1.297);
	ROS_INFO("Sending goal");
	ac.sendGoal(goal);
	ac.waitForResult();
	if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) ROS_INFO("succeeded");
...
//********************************************************************************
	...
	cout<<"开始识别脸"<<endl;
	stop(v_pub);
	tts("Please get ready. Next I do face recognition!");
	sleep(2);   //停留10s再开始人脸识别，或者导航离开再回来
...
//**********************************************************************出门导航
	...
	goal.target_pose.header.stamp = ros::Time::now();
	goal.target_pose.pose.position.x = 10.770;
	goal.target_pose.pose.position.y = 6.512;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(-0.023);
	ROS_INFO("Sending goal");
	ac.sendGoal(goal);
	ac.waitForResult();
	if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) ROS_INFO("succeeded");
	...
	goal.target_pose.header.frame_id = "map";       //这个点可以考虑直接标在门外，防止撞墙
	goal.target_pose.header.stamp = ros::Time::now();
	goal.target_pose.pose.position.x = 11.805;
	goal.target_pose.pose.position.y = 6.546; 
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(1.574);
	ROS_INFO("Sending goal");
	ac.sendGoal(goal);
	ac.waitForResult();
	if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) ROS_INFO("succeeded");
